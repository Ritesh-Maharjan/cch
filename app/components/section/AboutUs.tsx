...
const AboutUs = () => {
  const images = [
    ...
    "https://picsum.photos/seed/5/640/400",
    "https://picsum.photos/seed/6/640/400",
    "https://picsum.photos/seed/7/640/400",
  ];
  ...
  useEffect(() => {
    let rafId = 0;
    const maxTranslate = 50; // px of movement up/down

    function update() {
      if (!wrapperRef.current) return;
      const rect = wrapperRef.current.getBoundingClientRect();
      const windowHeight = window.innerHeight || 1;
      // map element center offset to [-1, 1]
  ...
  return (
    <section className="px-4 xl:px-0 pb-28 w-screen min-h-screen max-w-312.5 mx-auto">
      ...
      <div className="mt-14 grid grid-cols-1 text-[#033E6E] gap-x-4 gap-y-8 md:gap-x-16 lg:gap-x-32 lg:grid-cols-2 lg:grid-rows-[auto_1fr] md:px-4">
        {/* Parallax image wrapper: half height and overflow-hidden */}
        <div
          ref={wrapperRef}
          className="relative overflow-hidden h-150 md:h-[px] lg:row-span-2 order-2 lg:order-1"
          aria-hidden={false}
        >
          {/* move this inner container instead of the outer wrapper */}
          <div
